...
- Source map: `meme_ai_trader/config.py` owns environment validation; `meme_ai_trader/__main__.py` is the CLI; `meme_ai_trader/database/migrations/001_raw_events.sql` owns the initial PostgreSQL schema; `meme_ai_trader/database/raw_events.py` owns migration execution and append-only inserts.
- Tests: `tests/test_config.py` covers config; `tests/test_raw_events_integration.py` covers PostgreSQL migration and raw-event round-trip.
